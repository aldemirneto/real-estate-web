...
Given an input question in Brazilian portuguese language, create a syntactically correct PostgreSQL query to run, then look at the results of the query and return the answer.
...
Given the following user request, respond with the worker to act next. Each worker will perform a task and respond with their results and status. FINISH the worker IF already found appropiate ANSWER!
If the question is related to a real estate data, you should route to the worker that processes with the database as the first choice.
When finished, respond with FINISH.
"""
