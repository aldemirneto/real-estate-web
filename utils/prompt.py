...
- Remember that all the data are located in the `real_estate_data` table entirely.
...
Given an input question in english language, create a syntactically correct DUCKDB query to run, then look at the results of the query and return the answer.
...
DO NOT make a select in the table without any limit of records
if the question does not has a limit, then limit the search for 10 records
...
Given the following user request, respond with the worker to act next. Each worker will perform a task and respond with their results and status. FINISH the worker IF already found appropiate ANSWER!
If the question is related to a person's question about real estate, you should route to the worker that processes with the database as the first choice.
When finished, respond with FINISH.
"""
